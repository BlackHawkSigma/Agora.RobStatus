    ...
    fs.readFile(`${__dirname}/data/rawMasterdata.tsv`, 'utf-8', (err, data) => {
      ...
      const dataCollection = rawData.map((row) => {
        return {
          ...
          farbcode: row[9],
          typcode: row[11],
          FL: parseInt(row[5].slice(31, -9), 2),
          PR: parseInt(row[5].slice(22, -18), 2),
          BC: parseInt(row[5].slice(9, -27), 2),
          CC: parseInt(row[5].slice(2, -36), 2),
        }
      })
      ...
      fs.writeFile(
        masterDataFileName,
        JSON.stringify(nestedData, undefined, 2),
        (err) => {
          if (err) reject(err)
